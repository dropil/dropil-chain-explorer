    ...
    render(){
        ...
        if (this.props.loading){
            ...
            if (this.props.transactionExist){
                ...
                return <Container id="transaction">
                    <Helmet>
                        <title>Transaction {tx.txhash} | Dropil Chain Testnet Explorer</title>
                        ...
                        <CardBody>
                            <Row>
                                ...
                                <Col md={8} className="value">{(tx.tx.value.fee.amount.length > 0)?tx.tx.value.fee.amount.map((fee,i) => {
                                    return <span className="text-nowrap" key={i}>{((fee.amount/Meteor.settings.public.powerReduction)>=1)?(new Coin(parseFloat(fee.amount), fee.denom)).stakeString():(new Coin(parseFloat(fee.amount), fee.denom)).mintString()} </span>
